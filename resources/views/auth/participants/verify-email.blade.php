...
@section('content')
    ...
    <div class="card border-0 px-2 py-2" id="auth-card">
        <div class="card-body">
            <form action="{{ route('participants.verify-email.store', $participants->id_participants) }}" method="POST" enctype="multipart/form-data" class="confirm-submit">
                @csrf
                <div class="mb-4">
                    <label for="email" class="form-label">Email</label>
                    <div class="input-group">
                        <input type="email" class="form-control" name="email" id="email" autocomplete="email" value="{{ $participants->email }}" readonly>
    ...
    <div class="text-center mt-3 mb-0">
        ...
        <form action="{{ route('participants.verify-email.resend', $participants->id_participants) }}" method="POST" class="d-inline">
            @csrf
